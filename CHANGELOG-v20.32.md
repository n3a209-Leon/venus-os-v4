...
幹部跳窗的遮罩層雖為 `position:fixed; inset:0`，但其頂端始終落在內容區起點
而非螢幕頂端。這是因為 `position:fixed` 的定位基準會被祖先元素上的
`transform`、`filter`、`backdrop-filter`、`will-change`、`contain` 等屬性改寫
（形成 containing block）。

v20.27 與 v20.28 分別調整了對齊方式與 auto margin，皆只能改善而無法根治，
因為問題不在跳窗本身的樣式，而在它的定位基準。

## 處理

改用 `ReactDOM.createPortal()` 將跳窗掛載至 `document.body`。
如此它脫離整個 App 的元素層級，不論祖先具備何種屬性，
`position:fixed` 都能以視窗為基準，完整覆蓋畫面。

層級同時由 `zIndex:9000` 提高至 `99000`，確保不被底部導覽列遮住。

由於此跳窗全數使用 inline style（含 NERV 主題的三元判斷），
不依賴 `[data-theme=...]` 的後代選擇器，Portal 後外觀不受影響。

## 儲存驗證

v20.29 的延後批次寫入經真機確認資料完整無誤。

---

# v20.31（開場畫面底部暗帶）

`.spl-overlay::after` 的漸層遮罩底部收在 `rgba(2,5,8,.82)`，接近不透明。
疊在本就偏暗的海面構圖上，視覺上形成一塊實心黑帶，
與獎卡系統的滿版開場不一致。

該遮罩原本用於確保標題與狀態文字可讀，現改為：

- 底部漸層由 `.82` 降至 `.34`，起始點自 72% 後移至 86%
- 徑向暗角由 42% 起、`.3` 調整為 52% 起、`.22`
- `.spl-titles` 與 `.spl-status` 加上文字陰影，改由文字自身維持可讀性
...
殘餘的偶發卡頓已無法再靠讀原始碼縮小範圍，改為在 App 內建立量測機制，
以實際數據判斷成因，避免繼續以推測方式修改。

## 啟用方式

網址加上 `?perf=1` 啟用（會記住設定），加上 `?perf=0` 關閉。
啟用後右下角出現 ⏱ 按鈕，點擊即顯示報告。

**預設完全關閉，且關閉時不產生任何成本**：探針在旗標為否時立即 return，
`React.Profiler` 不會包裹根元件，儲存路徑的計時也不會執行。

## 量測內容

- **掉幀**：以 `requestAnimationFrame` 偵測間隔超過 50ms 的畫格
- **React 提交耗時**：透過 `React.Profiler` 記錄每次提交的 `actualDuration`
- **儲存耗時**：記錄整份資料序列化與 localStorage 同步寫入的時間

## 判定邏輯

每次掉幀都會回頭比對前後 32ms 內是否有 React 提交或儲存動作，
藉此將卡頓歸因到三類來源之一：

| 現象 | 判定 | 對應解法 |
|---|---|---|
| 掉幀伴隨高提交耗時 | React 重繪成本 | 列項 memo 化、狀態下放 |
| 提交很快但仍掉幀 | 繪製／版面成本 | 仍是 CSS |
| 掉幀緊接在儲存之後 | 儲存阻塞 | 再延後或分批寫入 |

報告會直接給出判定結論、掉幀次數與分佈、最嚴重的五次掉幀明細。
